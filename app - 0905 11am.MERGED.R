...
# =====================================================================
# BEGIN MERGED APP.R CONTENT (inlined on 2025-10-23)
# Notes:
# - Original `app - 0905 11am.R` content above is unmodified.
# - The original `source("app 0905 - 11am.R")` call is executed here after the base app code completes.
# - Any overlapping names are prefixed with `APP_R_`.
# - No second shinyApp() call is made here.
# =====================================================================

local({
  source_target <- "app 0905 - 11am.R"
  safe_normalize <- function(path) {
    tryCatch(normalizePath(path, mustWork = FALSE), error = function(e) NA_character_)
  }
  merged_path <- safe_normalize("app - 0905 11am.MERGED.R")
  target_path <- safe_normalize(source_target)
  if (!identical(target_path, merged_path)) {
    if (file.exists(source_target)) {
      source(source_target)
    } else {
      warning(sprintf("Expected file '%s' not found when executing merged app.R content.", source_target))
    }
  }
})

# =====================================================================
# END MERGED APP.R CONTENT
# =====================================================================
